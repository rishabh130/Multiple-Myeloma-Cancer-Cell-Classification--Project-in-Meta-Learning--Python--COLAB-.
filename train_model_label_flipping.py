...
def train(folder_names, val_folders, num_epochs , val_folder, gpu_no):
    best_val_acc=0.0
    # ref_image=imread(ref_image_name)
    ...
    for epoch in range(num_epochs):
        ...
        if test_epoch_acc > best_val_acc:
            torch.save(model, './5_fold_'+str(val_folder)+'_LF_best.pt')
    ...
    torch.save(model, './5_fold_'+ str(val_folder)+'_LF_final.pt')
...
if __name__ == '__main__':
    ...
    torch.cuda.set_device(int(gpu_no))
